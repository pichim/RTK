...
{  
    ...
    switch(m_msg[i].id){
        ...
        case(0x14): //UBX-NAV-HPPOSLLH
        {
            ...
            int32_t temp1 =    (uint32_t)m_msg[i].data[8]
                            | ((uint32_t)m_msg[i].data[9] << 8)
                            | ((uint32_t)m_msg[i].data[10] << 16)
                            | ((uint32_t)m_msg[i].data[11] << 24); //lon

            int32_t temp2 =    (uint32_t)m_msg[i].data[12]
                            | ((uint32_t)m_msg[i].data[13] << 8)
                            | ((uint32_t)m_msg[i].data[14] << 16)
                            | ((uint32_t)m_msg[i].data[15] << 24); //lat

            double lon = (double)temp1 / 10000000.0f;    // [deg]
            double lat = (double)temp2 / 10000000.0f;    // [deg]

            int32_t temp3 =    (uint32_t)m_msg[i].data[16]
    ...
    */
    ...
    while(remaining_bytes){
        ...
        if(header == header_ref){ //its a valid message
            ...
            if((remaining_bytes - (8 + m_msg[m_msg_index].length) < 0) || (m_msg[m_msg_index].class_ == 0) || (m_msg[m_msg_index].id == 0) || (m_msg[m_msg_index].length <= 8)){

#if GNSS_DO_PRINTF

                //printf("Message split at: 0x%02x\n",m_msg[m_msg_index].id);
                /*
                for(int ii = 0; ii < m_num_carry_bytes; ii++){
                    printf("%02x ",m_carry_bytes[ii]);
                }
                printf("\n");
                */
#endif
                n_carry_bytes = remaining_bytes;
                memcpy(buffer,buffer+offset,n_carry_bytes);
            ...
            m_msg[m_msg_index].ck_a = buffer[offset + 6 + i + 0];
...
bool GNSS::checksum(int msg_i){
    uint8_t a = 0;
    uint8_t b = 0;

    a += m_msg[msg_i].class_;
    b += a;

    a += m_msg[msg_i].id;
    b += a;

    a += (uint8_t)m_msg[msg_i].length;
    b += a;

    a += (uint8_t)(m_msg[msg_i].length >> 8);
    b += a;

    for(int i = 0; i < m_msg[msg_i].length; i++){
        a += m_msg[msg_i].data[i];
        b += a;
    }
    /*
